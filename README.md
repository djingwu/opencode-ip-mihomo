...
一个面向 **OpenCode Zen**、OpenAI 兼容客户端和 Anthropic 客户端的 **mihomo 出口代理网关**。项目将机场订阅节点、可选**提供免费节点**和 HTTP/HTTPS/SOCKS5 代理池统一纳入管理，并通过 ROTATOR 自动选择可用出口,它旨在防止 HTTP 429 速率限制，保证每个周期使用唯一的 IP 地址。
